...
export function handleAdCreated(event: AdCreatedEvent): void {
  ...
  if (advertiser == null) {
    ...
    advertiser.interests = new Array<BigInt>(event.params.adVector.length).fill(
      BigInt.fromI32(0)
    );
  } else {
    const diff: BigInt = event.params.expiresAt.minus(event.block.timestamp);
    advertiser.totalPaid = diff.times(BigInt.fromI32(1));
  }
