...
const provider = new ethers.WebSocketProvider(process.env.ZIRCUIT_WSS || "");
...
contract.on("DispatchId", (id) => {
    ...
    const command = `hyperlane status --relay --id ${id} --origin zircuit --destination fhenix --registry ./scripts/hyperlane-registry -k ${privateKey}`;
